...
public class SegmentsClient {
     ...
     */
    public SegmentsClient(MicroServiceTemplate microServiceTemplate) {
    ...
    */
    public GetSegmentsRs getSegments(long timeout) throws RestClientException {
        return getSegments().get(timeout);
    }

    /**
     * Запрос получения списка сегментов асинхронно
     * GET /auth/v1/segments
     *
     * @return 
     * @throws RestClientException в случае неудачи
    */
    public ClientRestResult<GetSegmentsRs> getSegments() throws RestClientException {
        return new ClientRestResult<>(getSegmentsInternal(), this::processStatusCodeErrors);
    }

    private ListenableFuture<ResponseEntity<GetSegmentsRs>> getSegmentsInternal() throws RestClientException {
        

        MicroServiceURIBuilder uriBuilder = JmsUriBuilder.service(applicationName)
            .path(cutComments("/auth/v1/segments"));

        RequestEntity requestEntity = RequestEntity
                  .method(HttpMethod.GET, uriBuilder.build())
                  .build();
        return microServiceTemplate.exchange(requestEntity,
                    new ParameterizedTypeReference<GetSegmentsRs>(){});
    }
